...
### A/B 对比（真实 Claude Code，12 轮会话，N=5）

同一任务、同一干净工作区，网关开 vs 关（关 = 只记账不投影）：

| 指标 | 网关关 | 网关开 | 差异 |
|---|---|---|---|
| 总 tokens（均值） | 756,463 | 479,142 | **−36.7%** |
| 成本（均值，示例单价） | $1.647 | $1.282 | **−22.2%** |

任务：读 3 个约 150 KB 的文件 + 9 次后续工具调用，共 12 次请求。OFF 臂方差较大
（61 万–98 万 tokens），中位数口径为 token −29.0% / 成本 −25.1%。成本按示例单价
$15/$75（缓存读 $1.5 / 写 $18.75）计算。复现：`./scripts/bench-ab.sh 5`。
