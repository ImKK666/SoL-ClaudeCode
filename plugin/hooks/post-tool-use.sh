...
# fused `then_run`. Disabled unless SOLPI_FUSION_COMMAND is set, so it is inert
# by default. Fail-safe: a failure never blocks or errors the tool.
set -uo pipefail

CMD="${SOLPI_FUSION_COMMAND:-}"
if [ -z "$CMD" ]; then
  exit 0
fi
...
node -e 'const body=process.argv[1]||"";process.stdout.write(JSON.stringify({hookSpecificOutput:{hookEventName:"PostToolUse",additionalContext:"[then_run] "+process.env.SOLPI_FUSION_COMMAND+"\n"+body}}))' "$OUT" 2>/dev/null || true
exit 0
